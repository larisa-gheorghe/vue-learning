...
- instalation: https://v3.vuejs.org/guide/installation.html#vue-devtools
...
- next we need to call a function in our app.js file called Vue.createApp()
        ...
        - the application it returns has the capability f keeping track of data and manipulating the document
        - this function has one argument, which is an object of configuration settings
- we will pass in an empty object: Vue.createApp({})    ( we'll add some settings later)
...
- we can add the vue extension: https://v3.vuejs.org/guide/installation.html#vue-devtools
...
- accessing the instance data:
        ...
        //app.js
                ...
                setTimeout(() => {
                vm.firstName = 'Bob'           <--- we can access directly firstName without mentioning vm.data.firstName
...
- outputting raw HTML:
        - cross site scripting(XSS): when malicious or harmful HTML is injected into the webpage causing unintended behavior in the browser
...
- listening to events: out of the box, vue supports native browser events such as clicks, key inputs, hovers etc.
      ...
      <button type="button" v-on:click="age--">Decrement</button>
        ...
        - i.e that mimics how the v-model works (the difference is that we can do more during the process if we need such control):
        //app.js
                methods: {
                        fullName() {
                        return `${this.firstName} ${this.lastName.toUpperCase()}`
...
- event modifiers:
        - calling event.preventDefault() is standard in most apps because you may not want the default browser behavior to affect your app
        - the event modifiers feature modify the way an event is handled
        - documentation: https://v3.vuejs.org/guide/events.html
...
- keyboard events and modifiers:
        - documentation for key modifiers: https://v3.vuejs.org/guide/events.html#key-modifiers
        - i.e:
        //app.js
                methods: {
        ...
        - if there isn't any name assigned, we can use the code assigned to a key by the browser: https://keycode.info/ (the event.code window)
        - i.e:
        <label for="">Middle Name</label>
        <input type="text" @keyup.enter="updateMiddleName">             <---- when enter is pressed
        <button type="button" v-on:click="increment">Increment</button>
        <button type="button" @click.ctrl="age--">Decrement</button>            <--- when Control is pressed while clicking the decrement button

- v-model Modifiers: there are 3
...
- computed properties = properties that are calculated in some sort of manner
        - when any oof the data properties are updated, you will render the template again onto the page
        ...
        - when a data property is updated: - first it will scan the function of the computed property and check what data properties you're using inside the function
        ...
        - vue knows when to call the function based on the data used inside the function, even if it isn't actively used
                - i.e: 
                //app.js
                computed: {
                        fullName() {
        ...
        - diffrences between methods and computed properties:
                ...
                - with cmputed properties they serve one purpose: to calculate a value; unlike methods, you are always expected to return a value

- watchers = watch your data for changes; whenever a change occurs, vue can run a function to execute additional logic
        ...
        - we use watchers to perform additional actions whenever a value changes; this is especially true when you want to perform asynchronous tasks
        ...
        //app.js
                ...
                watch: {
                        age(newVal, oldVal) {
                        setTimeout(() => {
                                this.age = 20           <--- after 3s, the age will revert back to 20, because the watcher function will run when we change the age
                        }, 3000)
                        }
                }

- binding classes:
        - documentation: https://v3.vuejs.org/guide/class-and-style.html
...
- conditional rendering: when elements are added or removed from the document based on a condition
        ...
        - to display multiple elements without hang to use numerous conditions we use the 'template' element, which serves as an invisible wrapper
...
- alternative solution for conditionally rendering content: v-show directive
...
- list rendering: vue can help us loop through an array of items to render a list of elements
        ...
        - when you're looping through properties in an object, the first name will represent the property's value and the second name will represent the key name
        ...
        - there is a way to keep track of items in arrays even if they move around: by binding the key attribute on the list item
        ...
        - without the key attribute, vue will move the data around, but not the elements themselves; tris process is called 'patching'
...
- documentation: https://v3.vuejs.org/guide/instance.html#lifecycle-hooks
...
- lifecycle:    
        Vue.createApp()              
        .mount()
                - the function where you pass in your settings and data for Vue to work with; during this process you will begin the lifecycle
        ...
        Initialize data and methods
                ...
                - before that happens, you have the opportunity to run some code    
                         beforeCreate hook      - at this point, the vue instance hasn't been fully created
        ...
        Instance Created
                ...
                - we are provided another lifecycle hook:
                        created                 - unlike before, we can access the data and methods inside the vue instance
                                                - vue is not mounted to any template, nor will anything be rendered onto the page
        Compile Template (el property)
                ...
                - once the template is compiled, vue will replace the content inside the element we selected with the mount() method with the compiled template
        ...
        vm.unmount()    
                ...
                - during this process there are two hooks we can use:
                        beforeUnmount           - is called before the instance is destroyed; your application is still fully functional and present on the page
                                                - you can access any of the data and methods in your vue app
                        unmounted               - triggered once the vue instance is destroyed
        ...
        Apply changes to the template
                - we are provided two hooks during this process (mainly used for debugging)
                        beforeUpdate            - gets triggered after the data has been updated but has not been applied to the template
...
- there are multiple versions of vue available for download, but all of them can be broken down into 2 categories
        ...
        - without the compiler (runtime onlty build):
                - advantages:   - 30% lighter than the full version;
                                - slightly faster, since Vue no longer has to compile the template
                - cons:         - difficult to read and manage code with plain JS
...
- documentation: https://v3.vuejs.org/guide/installation.html#explanation-of-different-builds
...
- Vue.h() function      - helps us to create the object in the correct format;
                        - will take care of filling in the blanks for us
                        - h is short for hyperscripts (you use JS to create HTML)
                        - it has 2 parameters: - the name of the element
                                                - the content inside the elements
- different versions of vue: https://cdnjs.com/libraries/vue
...
- we can create a component by calling the vm.component() function
        - it has 2 parameters:  - the name of the component ( kebob case convention is used for naming - replace space with a dash)
...
- then we can run: $ npm run start      or
                   $ npm run build
...
- usually, you will end up installing a minimum of three modules: 
        - Babel Core module:    - contains the core functionality; it does not determine what features we like to enable in the JS language
                                - it will take care of reading your code and compiling it into a code that the browser can understand
        - Babel Loader module:  - if we want to use Webpack and Babel together, this module is necessary
        - Babel Presets modules:- to provide the features we need
                                - list of presets available: https://babeljs.io/docs/en/presets
...
- creating a variable:  - i.e: $red: #CC3244;
...
- SASS comes with a set of functions for calculating color values:
        ...
        - list of functions you can use to change colors: https://sass-lang.com/documentation/modules/color
...
- documentation: https://github.com/postcss/postcss
- to isntall it: $ npm i post-loader --save-dev
...
- plugins available: https://www.postcss.parts/
...
- in the console we can see the name of the rule that we break: https://eslint.org/docs/rules/
        //.eslintrc.json
        {
            "extends": "eslint:recommended",
            "env": {
...
- the code base of an application can be broken into 2 parts:
        - code from external libraries: - can be considered code from libraries like Vue.js or Axios.js
                                        - it's code we didn't write but are using in our app
        - application code      - is the code we writen to get a functioning app
...
- documentation: https://vue-loader.vuejs.org/guide/scoped-css.html#scoped-css
...
- run $ npm i node-sass sass-loader --save-dev ( in care of errors you can try $ npm install -D sass-loader@^10 node-sass)
- i.e:
        //Greeting.vue
...
- documentation: https://cli.vuejs.org/guide/css.html#referencing-assets
...
- limitations of props: - we are not allowed to mutate props, because the parent component will not be notified of the change
...
- types available: https://v3.vuejs.org/guide/component-props.html#type-checks
...
- documentation: https://v3.vuejs.org/guide/component-slots.html
...
- we're not limited to one root element
- i.e:
//App.vue
